...
namespace eh_state
{
  ...
  {
    ehstate_t cs = from_dc(func_dsc, dc);
    ...
    {
      if (in_catch  &&  (try_dsc->low_level <= in_catch->high_level || try_dsc->high_level > in_catch->catch_level) )
      {
        continue;
      }

      if (state >= try_dsc->low_level && state <= try_dsc->high_level)
      {
        if (!try_begin.valid())
        {
          try_begin = try_dsc;
  ...
  )
  {
    ...
    for (try_rev_iterator try_dsc(*func_dsc, dc->ImageBase); try_dsc.valid(); try_dsc.next())
    {
      if (state > try_dsc->high_level && state <= try_dsc->catch_level)
      {
...
namespace eh_engine
{
  ...
  )
  {
    ...
    while (current_state > eh_state::EMPTY  &&  current_state != target_state  &&  unwind_entry[current_state].valid())
    {
  ...
  void* call_catch_block(::EXCEPTION_RECORD  const* const  unwind_exc_rec)
  {
  ...
  namespace aux_
  {
    ...
    , ::EXCEPTION_RECORD          const* const  exc_rec
    , funcframe_ptr_t             const&        func_frame
  ...
  {
    if ( eh::EXCEPTION_CODE_CPP == exc_rec->ExceptionCode  &&  EXCEPTION_OPCODE_THROW == exc_rec->ExceptionInformation[EXCPTR_OPCODE] )
    {
  ...
  void throw_exception_no_exception_object(::EXCEPTION_RECORD* const rec)
  {
    ...
    exc_rec.ExceptionFlags      = 0;  // this is continuable exception - if catchblock-handler would fill the necessary ptrs it will return ExceptionContinueExecution
  ...
  {
    image_base_t const image_base = dc->ImageBase;
    funcframe_ptr_t func_frame = eh_state::function_frame(func_dsc, dc, frame);
    ...
    try_iterator try_cur(*func_dsc, image_base);
    try_iterator try_end(*func_dsc, image_base);
    eh_state::try_range(func_dsc, dc, state, try_cur, try_end);
      ...
      if (EXCEPTION_OPCODE_THROW == exc_rec->ExceptionInformation[EXCPTR_OPCODE])
      {
        ...
        if (exc_object && exc_dsc)
        {
          for ( ; try_cur != try_end; try_cur.next())
          {
            ...
            {
              for (catch_iterator catch_block(*try_cur, image_base); catch_block.valid(); catch_block.next())
              {
                for (catchable_type_iterator catchable_type(*catchable_table_iterator(exc_dsc, image_base), image_base); catchable_type.valid(); catchable_type.next())
                {
                  if
                  (
                    eh_type::match
                    (
                      exc_dsc
                    , catchable_type
                    , catchable_typeinfo_iterator(*catchable_type, image_base)
                    , catch_typeinfo_iterator(*catch_block, image_base)
                    , catch_block->attributes
                    )
                  )
                  {
                    eh_type::copy_exception_object(func_frame, exc_object, catch_block, catchable_type, dc);
                    unwind_stack
                    (
                      frame
                    , exc_rec
                    , func_frame
                    , catch_handler_iterator(*catch_block, image_base)
                    , try_cur->low_level
                    , context
                    , func_dsc
                    , dc
...
next_try_block : ;
  ...
  // the SEH catching by (...) facility
        for ( ; try_cur != try_end; try_cur.next())
        {
          if (try_cur->low_level <= state && state <= try_cur->high_level)
          {
            catch_rev_iterator catch_block(*try_cur, image_base); 
            if ( catch_block.valid()  &&  !catch_typeinfo_iterator(*catch_block, image_base).valid() )  // check the last catch in the corresponding try is the '(...)'
            {
              unwind_stack
              (
                frame
  ...
  namespace aux_
  {
    ...
    msvc_internal_data::eh::func_descriptor const* const get_function_descriptor
    (
      ::DISPATCHER_CONTEXT   const* const  dc
    , image_base_t           const&        image_base
    )
    {
      return reinterpret_cast<msvc_internal_data::eh::func_descriptor const*>(image_base + *reinterpret_cast<rva_t*>(dc->HandlerData));
    }
  ...
  {
    func_descriptor_iterator const func_dsc(aux_::get_function_descriptor(dc, dc->ImageBase));

    //// check if this frame handler is responsible for incoming EXCEPTION_RECORD
    if ( eh::EXCEPTION_CODE_CPP == exc_rec->ExceptionCode ) // filter cpp-exception
    {
    ...
    else if ( STATUS_UNWIND_CONSOLIDATE != exc_rec->ExceptionCode )  // filter other no cpp-exception
    {
    ...
    if ( exc_rec->ExceptionFlags & EXCEPTION_UNWIND )
    {
      ...
      // unwinding the target frame
        if ( exc_rec->ExceptionFlags & EXCEPTION_TARGET_UNWIND && STATUS_UNWIND_CONSOLIDATE == exc_rec->ExceptionCode )
        {
          if ( ARRAYSIZE_EXCPTR_UNW == exc_rec->NumberParameters && EXCEPTION_OPCODE_STACK_CONSOLIDATE == exc_rec->ExceptionInformation[EXCPTR_OPCODE] )
          {
            unwind_frame
            (
              reinterpret_cast<funcframe_ptr_t>(exc_rec->ExceptionInformation[EXCPTR_UNW_FUNCTION_FRAME])
            , dc
            , func_dsc
      ...
      // unwinding the nested frames
        else
        {
          ...
          unwind_frame(func_frame, dc, func_dsc, target_state);
        }  
      }
    ...
    //  do not return if matching catch-block has been found (if standard NT exception dispatcher is used)
    //  or return and delegate the unwinding duties to the custom dispatching routine
    return ::ExceptionContinueSearch;
  }
