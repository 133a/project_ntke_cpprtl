    ...
    // the data used during the unwinding phase:
      ...
      while ( 1 )  // the frame-traversing loop
      {
        ...
        else // nested entry
        {
          ...
          if ( frame_handler )  // an exception-aware frame is found, let's check its' intentions
          {
...
stack_walker_unexpected :
  ...
  // do NOT use objects with the destructor semantic at this scope because of 'RtlRestoreContext()' just cuts the stack up
  __declspec ( noinline )
  ...
  // do NOT use objects with the destructor semantic at this scope because of '_CPPRTL_restore_context()' just cuts the stack up
  __declspec ( noinline )
  void stack_walk(::EXCEPTION_RECORD& original_exc_rec)
  {
    ...
    // otherwise we are being requested for the stack_walk() caller continuation
    return;
  }
